...
function ReviewWindow () {
  ...
  return (
        <Accordion activeKey={activeKey} onSelect={handleAccordionItemClick} style={{ height: '60vh' }}>
            <div className={'overflow-container'} style={{ maxHeight: '100%' }}>
                {roomSlots.map((roomSlot, roomSlotIndex) => (
                  roomSlot.getRooms().map((room, roomIndex) => {
                    ...
                    return (<Accordion.Item key={accordionItemKey} eventKey={accordionItemKey}>
                                <Accordion.Header className={'header-style list-item border-0'}>
                                  ...
                                  <span className={'review-text'} style={{ paddingLeft: 5 }}>
                                    {room.getReview()?.getGroupName()}
                                        ...
                                        <span className={'review-text'} style={{ paddingLeft: 5 }}>
                                    {roomSlot.getFormattedStartTime() + ' - ' + roomSlot.getFormattedEndTime()}
                                  </span>
                                        <span className={'review-text'} style={{ paddingLeft: 5 }}>
                                    {room.getName()}
                                  </span>
                                ...
                                <Accordion.Body>
                                    <Table responsive borderless className={'overflow-auto reviews-table'}>
                                        ...
                                        <tr>
                                            <td>{room.getReview()?.getModerator()?.getFirstName()}</td>
                                            <td>{room.getReview()?.getModerator()?.getLastName()}</td>
                                        ...
                                        {room.getReview()?.getReviewer().map((reviewer) => (
